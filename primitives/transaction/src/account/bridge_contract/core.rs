    ...
    pub fn validate_address_format(&self, chain_config: &ChainConfig) -> Result<(), BridgeError> {
        match chain_config.address_format {
            AddressFormat::Nimiq => {
                if self.target_address.len() != 20 {
                    return Err(BridgeError::InvalidAddress(
                        "Nimiq address must be 20 bytes".to_string(),
                    ));
                }
                Ok(())
            }
            ...
            AddressFormat::Bitcoin => {
                // Bitcoin addresses are variable length (typically 25-34 bytes for legacy,
                // or can be represented as bech32 strings)
                // For raw bytes, we accept 20-34 bytes
                if self.target_address.len() < 20 || self.target_address.len() > 34 {
                    return Err(BridgeError::InvalidAddress(
                        "Bitcoin address must be 20-34 bytes".to_string(),
                    ));
                }
                Ok(())
            }
            AddressFormat::Custom(ref format_name) => {
                ...
                // For custom formats, we just check it's not empty
...
/// the source chain's state. The relayer provides the oracle state hash
...
pub struct OutgoingTransaction {
    ...
    /// Oracle state hash that the proof is verified against
    /// Provided by the relayer from the oracle contract
    pub oracle_state_hash: AnyHash,
}
...
impl OutgoingTransaction {
    ...
    pub fn parse_burn_data(
        ...
        let address_value = result
            ...
            .get("target_address")
            .ok_or(BridgeError::InvalidRecipientData)?;
        let target_address = address_value.as_bytes()?;
...
/// Information about a burn transaction on the source chain.
///
/// This structure contains all the details of a transaction where assets
/// were burned on the source chain to initiate a cross-chain transfer.
/// The information is used to validate that the outgoing transaction
/// matches the actual burn that occurred.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BurnTransactionInfo {
    /// Raw transaction data from the source chain
    pub raw_data: Vec<u8>,
    /// Transaction hash on the source chain
    pub tx_hash: Blake2bHash,
    /// Amount of assets burned
    pub amount: Coin,
    /// Target address on destination chain (embedded in burn transaction, raw bytes)
    pub target_address: Vec<u8>,
    /// Nonce for transaction ordering
    pub nonce: u64,
    /// Block height where the burn occurred
    pub block_height: u32,
    /// Unix timestamp of the burn transaction
    pub timestamp: u64,
...
impl BurnTransactionInfo {
    /// Creates new burn transaction info with validation.
    ///
    pub fn new(
        raw_data: Vec<u8>,
        tx_hash: Blake2bHash,
    ...
    /// Validates the burn transaction info fields.
    ///
    /// Checks that all required fields are non-zero and data is present.
    ///
    pub fn validate(&self) -> Result<(), BridgeError> {
    ...
    /// Verifies that the stored hash matches the computed hash of raw data.
    ///
    pub fn verify_hash(&self) -> Result<bool, BridgeError> {
        if self.raw_data.is_empty() {
            return Err(BridgeError::InvalidDataLength);
        }
        let computed_hash = Blake2bHasher::default().digest(&self.raw_data);
        Ok(computed_hash == self.tx_hash)
    }

    /// Serializes the burn transaction info to bytes.
    ///
    pub fn to_bytes(&self) -> Result<Vec<u8>, BridgeError> {
        use nimiq_serde::Serialize;
        Ok(self.serialize_to_vec())
    ...
    /// Deserializes burn transaction info from bytes.
    ///
    pub fn from_bytes(data: &[u8]) -> Result<Self, BridgeError> {
        use nimiq_serde::Deserialize;
        Self::deserialize_all(data).map_err(|_| BridgeError::DeserializationError)
    ...
    /// Returns the transaction details as a tuple for BurnTransactionInfo.
    ///
    /// Returns (amount, target_address, nonce, block_height)
    pub fn transaction_details(&self) -> (Coin, &[u8], u64, u32) {
        (
            self.amount,
            &self.target_address,
            self.nonce,
...
impl MerkleProofValidator {
    ...
    pub fn extract_transaction_hash_with_endianness(
        ...
        let data_to_hash = if source_endianness != self.endianness {
            ...
            log::debug!(
                "Converting transaction data from {:?} to {:?}",
                source_endianness,
                self.endianness
            );
            tx_data
    ...
    pub fn update_oracle_address(
        ...
        log::info!("Updated oracle address to: {:?}", &new_oracle_address);
    ...
    pub fn set_endianness(&mut self, endianness: Endianness) {
        ...
        log::info!("Updated endianness to: {:?}", endianness);
    }
    ...
    /// Validates transaction details match between burn transaction and cross-chain request
    pub fn validate_transaction_details(
        ...
        // Parse the outgoing transaction to get its details
        let parsed = outgoing_tx.parse_burn_data(chain_config)?;

        // Validate amount matches
        if burn_info.amount != parsed.amount {
            log::warn!(
                "Amount mismatch: burn={}, outgoing={}",
                burn_info.amount,
                parsed.amount
            );
            return Ok(false);
        ...
        // Validate target address matches
        if burn_info.target_address != parsed.target_address {
            log::warn!(
                "Target address mismatch: burn={:?}, outgoing={:?}",
        ...
        if burn_info.nonce != parsed.target_nonce {
            log::warn!(
                "Nonce mismatch: burn={}, outgoing={}",
                burn_info.nonce,
                parsed.target_nonce
            );
            return Ok(false);
        ...
        if burn_info.block_height != parsed.burn_block_height {
            log::warn!(
                "Block height mismatch: burn={}, outgoing={}",
    ...
    /// Comprehensive proof verification that validates:
    /// 1. Oracle contract reference
    /// 2. State hash matching via direct oracle calls
    /// 3. Leaf node validation against burn transaction
    /// 4. Transaction detail validation (amount, address, height)
    pub fn verify_comprehensive_proof(
        &self,
        outgoing_tx: &OutgoingTransaction,
        burn_info: &BurnTransactionInfo,
        ...
        // Log proof verification started
        log::debug!(
            "Proof verification started: tx_id={:?}, burn_height={}, proof_depth={}",
            tx_id,
            burn_info.block_height,
            outgoing_tx.proof_depth()
        );

        // Step 1: Validate oracle contract reference
        log::debug!(
            "Transaction validation started: tx_id={:?}, stage=oracle_reference",
            tx_id
        );
        if !self.verify_oracle_reference(oracle_address)? {
            log::error!("Oracle reference validation failed");
            log::error!(
                "Proof verification failed: tx_id={:?}, error={}, details={}",
                tx_id,
                BridgeError::InvalidOracleSignature,
                "Oracle contract reference does not match configured oracle"
            );
            return Err(BridgeError::InvalidOracleSignature);
        }
        log::debug!(
            "Transaction validation successful: tx_id={:?}, stage=oracle_reference",
            tx_id
        );

        // Step 2: Validate transaction details match
        log::debug!(
            "Transaction validation started: tx_id={:?}, stage=transaction_details",
            tx_id
        );
        if !self.validate_transaction_details(burn_info, outgoing_tx, chain_config)? {
            log::error!("Transaction detail validation failed");
            log::error!(
                "Proof verification failed: tx_id={:?}, error={}, details={}",
                tx_id,
                BridgeError::InvalidMerkleProof,
                "Transaction details do not match between burn and outgoing transaction"
            );
            return Err(BridgeError::InvalidMerkleProof);
        }
        log::debug!(
            "Transaction validation successful: tx_id={:?}, stage=transaction_details",
            tx_id
        );

        // Step 3: Extract and validate leaf hash from burn transaction
        log::debug!(
            "Transaction validation started: tx_id={:?}, stage=leaf_hash",
            tx_id
        );
        let leaf_hash = self.extract_transaction_hash(&burn_info.raw_data)?;
        log::debug!(
            "Transaction validation successful: tx_id={:?}, stage=leaf_hash",
            tx_id
        );

        // Step 4: Validate proof structure
        log::debug!(
            "Transaction validation started: tx_id={:?}, stage=proof_structure",
            tx_id
        );
        self.validate_proof_structure(&outgoing_tx.merkle_proof)?;
        log::debug!(
            "Transaction validation successful: tx_id={:?}, stage=proof_structure",
            tx_id
        );

        // Step 5: Compute root hash from proof
        log::debug!(
            "Transaction validation started: tx_id={:?}, stage=root_computation",
            tx_id
        );
        let computed_root =
            self.compute_root_from_proof(&outgoing_tx.merkle_proof, leaf_hash.clone())?;
        log::debug!(
            "Transaction validation successful: tx_id={:?}, stage=root_computation",
            tx_id
        );

        // Step 6: Validate computed root against provided oracle state hash
        log::debug!(
            "Validating proof against oracle state: chain_id={}, block_height={}, oracle_hash={:?}",
            chain_id,
            burn_info.block_height,
        ...
        if computed_root != outgoing_tx.oracle_state_hash {
            log::error!(
                "Root hash mismatch: computed={:?}, oracle={:?}",
                computed_root,
                outgoing_tx.oracle_state_hash
            );
            log::error!(
                "Proof verification failed: tx_id={:?}, error={}, details={}",
                tx_id,
                BridgeError::RootHashMismatch,
                format!(
                    "Computed root {:?} does not match oracle state {:?}",
                    computed_root, outgoing_tx.oracle_state_hash
                )
            );
            return Err(BridgeError::RootHashMismatch);
        }

        // Step 7: Validate leaf node matches burn transaction
        log::debug!(
            "Transaction validation started: tx_id={:?}, stage=leaf_validation",
            tx_id
        );
        if !self.validate_leaf_node(&burn_info.raw_data, leaf_hash)? {
            log::error!("Leaf node validation failed");
            log::error!(
                "Proof verification failed: tx_id={:?}, error={}, details={}",
                tx_id,
                BridgeError::LeafHashMismatch,
                "Leaf hash does not match burn transaction data"
            );
            return Err(BridgeError::LeafHashMismatch);
        }
        log::debug!(
            "Transaction validation successful: tx_id={:?}, stage=leaf_validation",
            tx_id
        );

        log::info!(
            "Comprehensive proof verification successful for transaction {:?}",
            tx_id
        );

        // Log proof verification success
        log::info!(
            "Proof verification successful: tx_id={:?}, burn_height={}",
            tx_id,
            burn_info.block_height
        );
        ...
        while pc < self.operations.len() {
            ...
            match op {
                ...
                ValidationOp::Assert => {
                    ...
                    if condition == 0 {
                        log::error!("Validation assertion failed at operation {}", pc);
                        return Err(BridgeError::InvalidRecipientData);
                    }
                }
...
pub mod transaction_validation {
    ...
    use super::{
        AddressFormat, BridgeError, BurnTransactionInfo, ChainConfig, IncomingTransaction,
        OutgoingTransaction,
    };
    ...
    /// including raw data, amount, nonce, block height, timestamp, and chain ID.
    pub fn validate_burn_transaction_fields(
        burn_info: &BurnTransactionInfo,
    ) -> Result<(), BridgeError> {
        // Validate raw data is present
        if burn_info.raw_data.is_empty() {
            return Err(BridgeError::InvalidDataLength);
        }

        // Validate raw data size (max 10KB)
        if burn_info.raw_data.len() > 10_000 {
            return Err(BridgeError::InvalidDataLength);
        }

        // Validate amount is positive
        if burn_info.amount == Coin::ZERO {
            return Err(BridgeError::InvalidAmount);
        }

        // Validate nonce is non-zero
        if burn_info.nonce == 0 {
            return Err(BridgeError::NonceAlreadyUsed);
        }

        // Validate block height is non-zero
        if burn_info.block_height == 0 {
            return Err(BridgeError::InvalidValidityHeight);
        }

        // Validate timestamp is non-zero
        if burn_info.timestamp == 0 {
            return Err(BridgeError::InvalidTimestamp);
        }

        // Validate chain ID is non-zero
        if burn_info.source_chain_id == 0 {
            return Err(BridgeError::InvalidChainId);
        }

        // Verify hash matches raw data
        let computed_hash = Blake2bHasher::default().digest(&burn_info.raw_data);
        if computed_hash != burn_info.tx_hash {
            return Err(BridgeError::InvalidMerkleProof);
        }
    ...
    /// Validates all aspects of a burn transaction with comprehensive checks.
    ///
    /// Performs field validation, amount validation, address validation,
    /// validity height validation, and data integrity validation.
    pub fn validate_burn_transaction_comprehensive(
        burn_info: &BurnTransactionInfo,
        chain_config: &ChainConfig,
        current_height: u32,
    ) -> Result<(), BridgeError> {
        // Validate all fields
        validate_burn_transaction_fields(burn_info)?;

        // Validate amount with reasonable maximum (e.g., 1 billion coins)
        let max_amount = Coin::from_u64_unchecked(1_000_000_000_000_000); // 1B coins
        validate_amount(burn_info.amount, Some(max_amount))?;

        // Validate address format
        validate_address_format(&burn_info.target_address, chain_config)?;

        // Validate block height is reasonable (not too far in the past or future)
        if burn_info.block_height > current_height + 1000 {
            return Err(BridgeError::InvalidValidityHeight);
        }

        // Validate data integrity
        validate_data_integrity(&burn_info.raw_data, &burn_info.tx_hash)?;
